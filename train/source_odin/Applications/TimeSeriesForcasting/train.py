...
def train(_link):
    '''
    train LSTM model using data from csv file
    save trained model to *.h5 file and transformer to *.scaler file (for latter use)
    :param _link: csv file path
    :return: None
    '''
    ...
    #INFERENCE
    start = time.time()
    raw_data, model_name = loadData('data\\all_2.csv', freq='15T')
    ...
    window = train_data[-config.time_steps-1:]
    # window = train_data
